...
RSpec.describe Game do
  context '#new' do
    ...
    it 'stops program if file is empty string' do
      expect do
        Game.new('')
      end.to raise_error(SystemExit).and output("File input is empty.\n").to_stderr
    end
  end
  ...
  context '#process_rounds' do
    it 'detects bad formatted input 1' do
      ...
      end.to raise_error(SystemExit).and output("\"Joe1\" can't be processed.\n").to_stderr
    end
    ...
    it 'detects bad formatted input 2' do
      ...
      expect do
        processor.process_rounds
      end.to raise_error(SystemExit).and output("\"Joe 1 10\" can't be processed.\n").to_stderr
    end
    ...
    it 'detects negative scores' do
      ...
      expect do
        processor.process_rounds
      end.to raise_error(SystemExit).and output("Invalid score on \"Joe -1\".\n").to_stderr
    end
    ...
    it 'detects scores bigger than 10' do
      ...
      expect do
        processor.process_rounds
      end.to raise_error(SystemExit).and output("Invalid score on \"Joe 11\".\n").to_stderr
    end
    ...
    it 'detects invalid letters or words on scores' do
      ...
      expect do
        processor.process_rounds
      end.to raise_error(SystemExit).and output("Invalid score on \"Joe A\".\n").to_stderr
    end
    ...
    it 'returns a Hash with players as keys and an array of scores as values' do
      ...
      processor = Game.new(txt)
      processor.process_rounds
      expect(processor.players_rounds).to eq({ 'Joe' => %w[1 9], 'Marcus' => ['F'] })
    end
  end
end
